...
@section('content')
    <section class="grid md:grid-cols-2 gap-10 items-center mb-12"
             x-data="{ show: false }"
        ...
        <div>
            ...
            <div class="flex flex-wrap gap-3 mb-6 text-xs">
                <a href="{{ route('login.siswa') }}" class="inline-flex items-center px-4 py-2 rounded-full bg-primary text-white font-semibold shadow-sm hover:bg-primary/90">
                    Masuk Siswa
                </a>
                <!-- <a href="{{ route('login.admin') }}" class="inline-flex items-center px-4 py-2 rounded-full border border-primary text-primary font-semibold hover:bg-primary/5">
                    Admin / Pembimbing
                </a> -->
                <a href="{{ route('register.dudi') }}" class="inline-flex items-center px-4 py-2 rounded-full border border-slate-200 text-slate-700 hover:bg-slate-50">
                    Mitra DUDI
                </a>
            </div>
            ...
            <div class="flex items-center justify-between mb-3">
                ...
                @if (!$industries->isEmpty())
                    <div class="flex items-center gap-2 text-[11px] text-slate-500">
                        <button type="button" @click="prev()" class="w-6 h-6 flex items-center justify-center rounded-full border border-slate-200 hover:bg-slate-50">
                            <span>&larr;</span>
                        </button>
                        <button type="button" @click="next()" class="w-6 h-6 flex items-center justify-center rounded-full border border-slate-200 hover:bg-slate-50">
                            <span>&rarr;</span>
                        </button>
                    </div>
                @endif
                ...
                <div class="text-xs">
                    @foreach ($industries as $i => $industri)
                        <div x-show="index === {{ $i }}" x-cloak
                            ...
                            <div>
                                <div class="font-semibold text-slate-900 text-xs">{{ $industri->nama_industri ?? $industri->nama ?? 'Industri' }}</div>
                            ...
                            @if (!empty($industri->wilayah))
                                <span class="text-[11px] px-2 py-1 rounded-full bg-primary/5 text-primary font-medium">{{ $industri->wilayah }}</span>
                            @endif
                        </div>
                    @endforeach
                    ...
                    <div class="mt-2 flex items-center justify-between text-[11px] text-slate-500">
                        ...
                        <a href="{{ route('public.industri') }}" class="text-primary hover:underline">Lihat semua industri &rarr;</a>
                    </div>
                </div>
            @endif
        </div>
                ...
                @foreach ($infos as $info)
                    <article class="bg-white border border-slate-100 rounded-xl p-4 flex flex-col justify-between">
                        <div>
                            ...
                            <h3 class="font-semibold text-slate-900 mb-1 line-clamp-2">{{ $info->judul ?? $info->title }}</h3>
                            @if (!empty($info->isi))
                                <p class="text-[11px] text-slate-600 line-clamp-3">{{ strip_tags($info->isi) }}</p>
                            @endif
                        </div>
                        <div class="mt-3 text-right">
                            <a href="{{ route('public.info') }}" class="text-[11px] text-primary hover:underline">Selengkapnya</a>
                        </div>
                    </article>
                @endforeach
            </div>
