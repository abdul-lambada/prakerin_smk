...
<head>
    ...
    <title>{{ $title ?? ($settings['app_name'] ?? 'PKL SMK') }}</title>

    @if (!empty($settings['app_logo']))
        <link rel="icon" type="image/png" href="{{ asset($settings['app_logo']) }}">
    @else
        <link rel="icon" type="image/svg+xml" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'%3E%3Ccircle cx='16' cy='16' r='16' fill='%232563eb'/%3E%3Ctext x='16' y='21' text-anchor='middle' font-size='14' fill='white' font-family='Arial'%3EPK%3C/text%3E%3C/svg%3E">
    @endif
...
<body class="bg-slate-50 text-slate-800 min-h-screen flex flex-col">
    <header class="bg-white shadow-sm sticky top-0 z-30">
            ...
            <div class="flex items-center justify-between h-16">
                <a href="{{ route('public.home') }}" class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-full bg-white border border-primary/20 flex items-center justify-center overflow-hidden">
                        ...
                        @else
                            <span class="text-primary font-bold text-sm">PKL</span>
                        @endif
                    </div>
                    <div>
                        <div class="text-sm font-semibold uppercase tracking-wide text-primary">{{ $settings['school_name'] ?? 'SMK' }}</div>
                        <div class="text-xs text-slate-500">{{ $settings['app_name'] ?? 'Sistem Informasi PKL' }}</div>
                    </div>
                </a>
                ...
                <nav class="hidden md:flex items-center gap-6 text-sm font-medium">
                    ...
                    <a href="{{ route('public.industri') }}" class="hover:text-primary {{ request()->routeIs('public.industri') ? 'text-primary' : 'text-slate-700' }}">Industri PKL</a>
                    <a href="{{ route('public.info') }}" class="hover:text-primary {{ request()->routeIs('public.info') ? 'text-primary' : 'text-slate-700' }}">Info PKL</a>
                    <a href="{{ route('public.about') }}" class="hover:text-primary {{ request()->routeIs('public.about') ? 'text-primary' : 'text-slate-700' }}">Tentang PKL</a>
        ...
        <div class="md:hidden" x-show="mobileMenu" x-cloak>
            <div class="px-4 pb-3 space-y-1 text-sm font-medium bg-white border-t border-slate-100">
                ...
                <a href="{{ route('public.industri') }}" class="block py-2 {{ request()->routeIs('public.industri') ? 'text-primary' : 'text-slate-700' }}">Industri PKL</a>
                <a href="{{ route('public.info') }}" class="block py-2 {{ request()->routeIs('public.info') ? 'text-primary' : 'text-slate-700' }}">Info PKL</a>
                <a href="{{ route('public.about') }}" class="block py-2 {{ request()->routeIs('public.about') ? 'text-primary' : 'text-slate-700' }}">Tentang PKL</a>
    ...
    <footer class="border-t border-slate-200 bg-white mt-10">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-500 gap-2">
            <div>
                &copy; {{ date('Y') }} {{ $settings['school_name'] ?? 'SMK' }} - Sistem Informasi PKL.
            </div>
