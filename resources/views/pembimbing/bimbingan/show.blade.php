...
<div class="card mb-4">
    <div class="card-body">
        <dl class="row mb-0">
            ...
            <dd class="col-sm-9">{{ $pembimbing->nama_lengkap }}</dd>
            ...
            <div class="d-flex mb-3 {{ $isFromPembimbing ? 'justify-content-end' : 'justify-content-start' }}">
                <div class="px-3 py-2" style="max-width: 70%; border-radius: 18px; {{ $isFromPembimbing ? 'background:#d1e7ff; text-align:right;' : 'background:#e9ecef;' }}">
                    <div class="small text-muted mb-1">
                        ...
                        {{ $isFromPembimbing ? 'Saya (Pembimbing)' : 'Siswa' }}
                    </div>
                    @if($item->judul)
                        <div class="mb-1">
                            <span class="badge
...
<div class="card mb-4">
    ...
    <div class="card-body">
        ...
        <form method="POST" action="{{ route('pembimbing.bimbingan.store', $tempat) }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <small class="form-text text-muted">Bisa mengirim catatan revisi, contoh laporan, dsb. (max 4 MB).</small>
                @error('file')
                    <span class="invalid-feedback d-block" role="alert"><strong>{{ $message }}</strong></span>
                @enderror
            </div>

            <a href="{{ route('pembimbing.monitoring.index') }}" class="btn btn-secondary">Kembali</a>
